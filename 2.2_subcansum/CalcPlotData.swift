...
@Observable class CalculatePlotData {
    ...
    func plotYEqualsX() async
    {
        ...
        let bob: [Int] = [1, 2, 5, 10, 20, 50, 100, 200, 500, 1000, 10000, 100000, 1000000, 10000000, 100000000]
        for N in bob  {
        ...
        //create y values here
            let sum1 = await sumsModel.getSum1(N: N)
            let sum3 = await sumsModel.getSum3(N: N)
            let quotient = abs(sum1-sum3)/sum3
        let y = -log10(quotient)
